...
def create_chunk(doc):
    ...
    for kdx, sent in enumerate(doc.sentences):
        ...
        for word in sent.words:
            ...
            elif (word.dependency_relation == "conj" and word.governor == 1) or (
                word.dependency_relation == "conj"
                and (word.governor in root_sub or word.governor in conj_sub)
            ):
        ...
        for jdx, word in enumerate(sent.words):
            if (word.dependency_relation == "root") and (
                ("root" in instr_depend_sub) or ("parataxis" in instr_depend_sub)
            ):
                ...
                if len(instr_lemma_sub) >= 2:
                    ...
                    if (
                        (
                            ("advmod" in instr_depend_sub)
                            or ("xcomp" in instr_depend_sub)
                        )
                        and ("obj" not in instr_depend_sub)
                        and ("obl" not in instr_depend_sub)
                        and ("nmod" not in instr_depend_sub)
                    ):
                        if (
                            (instr_lemma[-1][-1] == "and")
                            or (instr_lemma_sub[0] == "and")
                            or (instr_lemma[-1][-1] == "then")
                            or (instr_lemma_sub[0] == "then")
                        ):
            ...
            elif (word.dependency_relation == "parataxis") and (
                ("root" in instr_depend_sub) or ("parataxis" in instr_depend_sub)
            ):
                if len(instr_lemma_sub) >= 2:
                    """ check for the special cases of turning and the word "and","then" """
